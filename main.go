...
// NONE reppresents the initial status of a terminal
// BACK is the previous page
...
func back(oldPage Page, changePage chan Page) {
	if oldPage == WELCOME || oldPage == NONE {
		changePage <- OPTIONS
	} else {
		changePage <- oldPage
	}
		...
		for {
			...
			case WELCOME:
				...
				go func() {
					...
					if currentPage != TERMINALSMALL {
						width, height := termbox.Size()
						if width < TERMWIDTH || height < TERMHEIGHT {
							changePage <- TERMINALSMALL
						} else {
							changePage <- OPTIONS
						}
					}
			...
			case TERMINALSMALL:
				drawSmallTerminal(termbox.Size())
			case OPTIONS:
				startPlay := drawOptions(keyPressed, &gameOptions, "start")
				if startPlay {
					go func() { changePage <- GAME }()
				}
			case GAME:
				go play(keyPressed, gameOptions)
			case BACK:
				go back(oldPage, changePage)
			}
			if currentPage != TERMINALSMALL && currentPage != BACK {
				oldPage = currentPage
			}
		}
	}()
